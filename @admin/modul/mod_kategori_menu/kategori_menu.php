...
<style>.ui-widget-header{background:none;border:none;}</style>
		...
		<?php
		$aksi="modul/mod_kategori_menu/aksi_kategori_menu.php";
		$module="kategori_menu";
		switch($_GET['act']){
			default:
		?>
		
		<article class="module width_full">
			<header><h3 class="tabs_involved">Kategori Menu</h3>
		...
		<div class="tab_container">
			...
			<?php 	
				$no=1;
				$kategori_menu = mysql_query("SELECT * FROM kategori_menu ORDER BY id DESC");
				while($b=mysql_fetch_array($kategori_menu)){
		...
		<article class="module width_full">
			<header><h3 style="color: Red;">Add kategori menu</h3></header>
		...
		<?php break; 
		case"edit":
			$kategori_menu = mysql_query("SELECT * FROM kategori_menu WHERE id='$_GET[id]'");
			$r=mysql_fetch_array($kategori_menu);
		?>
		
		<article class="module width_full">
			<header><h3>Edit kategori menu</h3></header>
