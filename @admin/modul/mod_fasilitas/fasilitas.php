...
<style>.ui-widget-header{background:none;border:none;}</style>
		...
		<article class="module width_full">
			<header><h3 class="tabs_involved">fasilitas</h3>
		...
		<div class="tab_container">
			...
			<?php 	
				$no=1;
				$fasilitas = mysql_query("SELECT * FROM fasilitas ORDER BY id_fasilitas DESC");
				while($b=mysql_fetch_array($fasilitas)){
				...
				<tr>  
    				...
    				<td align="center"><img height="70px" src="../joimg/fasilitas/<?php echo"$b[gambar]" ?>"></td> 
    				<td><?php echo"$b[nama_fasilitas_ina]" ?></td> 
    				<td width="90px"><?php echo $tanggal; ?></td> 
    				<td align="center">
					<a href="<?php echo"?module=$module&act=edit&id=$b[id_fasilitas]";?>"><input type="image" src="images/icn_edit.png" title="Edit"></a>&nbsp;&nbsp;&nbsp;&nbsp;
					<a href="<?php echo"?module=subfeature&id_fasilitas=$b[id_fasilitas]";?>"><input type="image" src="images/+.png" title="Add Facilities" style="width: 20px;"></a>&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;<a href="<?php echo"?module=subimages&id=$b[id_fasilitas]";?>"><input type="image" src="images/img1.png" title="Add images" style="width: 20px;"></a>
					&nbsp;&nbsp;&nbsp;&nbsp; <a href="<?php echo"$aksi?module=$module&act=hapus&id=$b[id_fasilitas]";?>" onclick="return confirm('Apakah anda yakin menghapus data ini?');"><input type="image" src="images/icn_trash.png" title="Trash"></a>
					</td> 
				</tr> 
			<?php $no++; } ?>
		...
		<article class="module width_full">
			<header><h3 style="color: Red;">Add fasilitas</h3></header>
		...
		<?php break; 
		case"edit":
			$fasilitas = mysql_query("SELECT * FROM fasilitas WHERE id_fasilitas='$_GET[id]'");
			$r=mysql_fetch_array($fasilitas);
		?>
		
		<article class="module width_full">
			<header><h3>Edit fasilitas</h3></header>
			<form method='POST' enctype='multipart/form-data' action='<?php echo "$aksi"; ?>?module=<?php echo $module;?>&act=update'>
			<div class="col-lg-12">
				<input type='hidden' name='id' value='<?php echo"$r[id_fasilitas]" ?>'>
                    <div class="panel panel-default">
                        <!-- /.panel-heading -->
                        <div class="panel-body">
                            <!-- Nav tabs -->
                            ...
                            <div class="tab-content">
                                <div class="tab-pane fade active in" id="ina">
									<div class="module_content">
										<fieldset><label>Judul</label><br /><br />
											<input style="width:96%; margin-bottom:8px;" name="judul_ina" type="text" value="<?php echo"$r[nama_fasilitas_ina]" ?>">
										</fieldset>
										<fieldset><label>Deskripsi</label><br /><br />
											<textarea name="isi_ina" id="jogmce"><?php echo"$r[isi_fasilitas_ina]" ?></textarea>
										</fieldset>
									<div class="clear"></div>
									</div>
                                </div>
                                <div class="tab-pane fade" id="en">
									<div class="module_content">
										<fieldset><label>Tittle</label><br /><br />
											<input style="width:96%; margin-bottom:8px;" name="judul_en" type="text" value="<?php echo"$r[nama_fasilitas_en]" ?>">
										</fieldset>
										<fieldset><label>Description</label><br /><br />
											<textarea name="isi_en"><?php echo"$r[isi_fasilitas_en]" ?></textarea>
										</fieldset>
									</div>	
                                </div>
                            </div>
					...
					<fieldset style="float:left; width:30%; margin-right: 3%;"> <!-- to make two field float next to one another, adjust values accordingly -->
						&nbsp;&nbsp;<img width="270px" src="../joimg/fasilitas/<?php echo"$r[gambar]" ?>">
