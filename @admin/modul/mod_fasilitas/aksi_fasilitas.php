...
if ($module=='fasilitas' AND $act=='hapus'){
	...
	$cek = mysql_fetch_array(mysql_query("SELECT gambar FROM fasilitas WHERE id_fasilitas='$id'"));
	if($cek['gambar']!=''){
	mysql_query("DELETE FROM fasilitas WHERE id_fasilitas='$_GET[id]'");
    unlink("../../../joimg/fasilitas/$cek[gambar]");   
  } else { 
	mysql_query("DELETE FROM fasilitas WHERE id_fasilitas='$id'");
	}
...
// Input header
elseif ($module=='fasilitas' AND $act=='insertnew'){
  ...
  if (!empty($lokasi_file)){
    ...
    mysql_query("INSERT INTO fasilitas(nama_fasilitas_en,
									 nama_fasilitas_ina,
									 seo_en,
									 seo_ina,
									 isi_fasilitas_ina,
...
elseif ($module=='fasilitas' AND $act=='update'){
  ...
  $judul_seo_ina      = seo_title($_POST[judul_ina]);
	...
	$tampil=mysql_query("SELECT * FROM fasilitas WHERE id_fasilitas='$_POST[id]'");
	$ex=mysql_fetch_array($tampil);
		if($ex['gambar'] != ''){
		unlink("../../../joimg/fasilitas/$ex[gambar]");
		}
	
	UploadFasilitas($nama_file_unik);
  
    mysql_query("UPDATE fasilitas SET nama_fasilitas_en 	= '$_POST[judul_en]',
								   nama_fasilitas_ina 	= '$_POST[judul_ina]',
									seo_en 			= '$judul_seo_en',
									seo_ina 		= '$judul_seo_ina',
									isi_fasilitas_en 	= '".mysql_real_escape_string($_POST['isi_en'])."',
									isi_fasilitas_ina 	= '".mysql_real_escape_string($_POST['isi_ina'])."',
									tanggal			= NOW(),
...
// Update header
if ($module=='fasilitas' AND $act=='update_header'){
